...
class ImportBaseTest < Test::Unit::TestCase
  ...
  def test_should_remove_slash_and_author_name_from_title
    assert_equal "Foo. Di Bar", @importer.clean_title("Foo / di Bar")
    assert_equal "Bar. De Foo", @importer.clean_title("Bar / de Foo")
    assert_equal "Zork. Foo Bar", @importer.clean_title("Zork / Foo Bar")
  end
  
  def test_should_keep_info_after_author
    assert_equal "Yepeto. Di Roberto Cossa; traduzione di Nestor Garay",
      @importer.clean_title("Yepeto / di Roberto Cossa ; traduzione di Nestor Garay")

    assert_equal "Foo. Di Bla Bla. Sei testi di autori contemporanei.",
